...
const rom uint8_t start_config[] = 
{
...
static void FactorySettingsReadAndWrite( ParametersBuffer* parameters_buffer )
{
    static const uint8_t SIZE_BUF_AND_ADDR_MAGIC_VAL = sizeof( start_config ) / sizeof( start_config[0] ); 
    static const uint8_t MAGIC_VALUE = 0xAA;
    static const uint8_t START_ADDR_EEPROM = 0U;
    
    uint8_t count;
    
    if( DataEERead( SIZE_BUF_AND_ADDR_MAGIC_VAL ) == MAGIC_VALUE ) //уже есть записанные данные
    {
        Read_buffer_EEPROM ( START_ADDR_EEPROM, parameters_buffer, SIZE_BUF_AND_ADDR_MAGIC_VAL );
    }
    else    //запись "заводских" установок 
    {
        for ( count = 0; count < SIZE_BUF_AND_ADDR_MAGIC_VAL; count++ ) 
        {
            DataEEWrite( START_ADDR_EEPROM + count, start_config[count] );
            parameters_buffer->buffer[count] = start_config[count];
        }
        DataEEWrite( SIZE_BUF_AND_ADDR_MAGIC_VAL, MAGIC_VALUE ); //write magic value to next EEPROM cell (after last element of buffer)
    }
}
