...
#define QUANTITY_OF_MENU_PARAMETERS	 ( sizeof(Menu_items_thresholds) / Menu_items_thresholds[0] - 1 ) 
#define INCREMENT	1
#define DECREMENT	0
...
/* Функция сохранения параметра меню, если он изменился */
...
void Menu_Key_handler(KeyState* key_str)
{
	...
	((M_Mngm_Get_state() == STATE_WAITING) || ((M_Mngm_Get_state() == STATE_BURNING) && ((key_str->M_LI_Code_Key & NO_KEY_PRESSED) 

            == CODE_KEY_DEC_LONG__PRESSED)))) //произошло длительное нажатие какой-либо кнопки
	{
