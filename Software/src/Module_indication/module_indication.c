...
void M_Ind_Run (void)
{
	switch(M_Ind_State)
	{
		...
		case STATE_VIEW_NUM_PAGE_MENU :		//отображение номера страницы пользовательского меню
            ...
            M_Ind_blinking_7segm(LEFT_SYMBOL_WITH_DOT, NO_NEED_BLINK, 1);
		...
		case STATE_VIEW_PARAMETER_MENU :	//отображение параметра меню на выбранной странице (мигаем Ж+З)
			...
			M_Ind_LED_blinking(BLINKING_YELLOW, INDEX_OF_BICOLOR_LED);
			LED_FAN_PIN = OFF;
			M_Ind_blinking_7segm(LEFT_SYMBOL_WITH_DOT, NO_NEED_BLINK, 0);
            M_Ind_blinking_7segm(LEFT_SYMBOL_NO_DOT, NEED_BLINK, 1);
            M_Ind_view_num_page_menu();
            M_Ind_view_parameter_menu();
			M_Ind_TimerAutoExit();
		break;
		...
		break;
		case STATE_VIEW_ASH_CLEAN:
            M_Ind_Previous_State = STATE_VIEW_ASH_CLEAN;
            M_Ind_view_text(IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_A, 
			IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_S, IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_H, blink_mode, 0);
			M_Ind_view_text(IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_C, 
			IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_L, IT_IS_A_TEXT_SYM_AREA + OFFSET_TO_SYMBOL_E, blink_mode, 1);
            break;
...
static void M_Ind_view_num_page_menu(void) 
{
	...
	else
	{
        ...
        //M_Ind_data[0].dot[LEFT_INDICATOR] = M_Ind_data[0].const_dot[INDEX_IS_DOT];
        M_Ind_data[0].dot[LEFT_INDICATOR] = M_Ind_data[0].const_dot[M_Ind_data[0].Flag_Blink_seven_segm];
    }
}
